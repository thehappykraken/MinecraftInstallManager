...
def test_searchall_geyser_plugins(geyser_repository, plugin_name):
    versions = geyser_repository.searchAll(plugin=Plugin(plugin_name))
    assert versions is not None
    assert len(versions) > 0
    assert any(version.repository == geyser_repository for version in versions)
    assert all(version.version is not None for version in versions)

@pytest.mark.parametrize("plugin_name", geyser_plugins)
def test_geyser_repository_install(geyser_repository, plugin_name, tmp_path):
    plugin = Plugin(plugin_name)
    versions = geyser_repository.search(plugin=plugin)
...
@pytest.mark.parametrize("plugin_name", geyser_plugins)
def test_geyser_plugin_installed_versions(plugin_name, tmp_path):
    plugin = Plugin(plugin_name)
